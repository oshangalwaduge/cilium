...
type DNSProxy struct {
	...
	lookupTargetDNSServer func(w dns.ResponseWriter) (serverIP net.IP, serverPort, serverProtocol uint16, addrStr string, err error)
...
// portProtoToSelectorAllow maps port numbers to selectors + rules
type portProtoToSelectorAllow map[uint16]map[uint16]CachedSelectorREEntry
...
type perEPRestored map[uint64]map[uint16]map[uint16][]restoredIPRule
...
func (p *DNSProxy) checkRestored(endpointID uint64, destPort, destProto uint16, destIP string, name string) bool {
	portMap, exists := p.restored[endpointID][destProto]
	if !exists {
		return false
	}
	ipRules, exists := portMap[destPort]
	if !exists {
		return false
	}
...
func (p *DNSProxy) GetRules(endpointID uint16) (restore.DNSRules, error) {
	...
	portProtoToSelRegex := make(map[uint16]map[uint16][]selRegex)
...
func (p *DNSProxy) RestoreRules(ep *endpoint.Endpoint) {
	...
	restoredRules := make(map[uint16]map[uint16][]restoredIPRule, len(ep.DNSRules))
...
func (allow perEPAllow) removeAndReleasePortRulesForID(cache regexCache, endpointID uint64, destPort, destProto uint16) {
...
func (allow perEPAllow) setPortRulesForID(cache regexCache, endpointID uint64, destPort, destProto uint16, newRules policy.L7DataMap) error {
	if len(newRules) == 0 {
	...
	if !exist {
		epPorts = make(portProtoToSelectorAllow)
		epPorts[destProto] = make(map[uint16]CachedSelectorREEntry)
		allow[endpointID] = epPorts
	}
	epPorts[destProto][destPort] = cse
	return nil
}
...
func (allow perEPAllow) setPortRulesForIDFromUnifiedFormat(cache regexCache, endpointID uint64, destPort, destProto uint16, newRules CachedSelectorREEntry) error {
	if len(newRules) == 0 {
		allow.removeAndReleasePortRulesForID(cache, endpointID, destPort, destProto)
		return nil
	}
...
func (allow perEPAllow) getPortRulesForID(endpointID uint64, destPort, destProto uint16) (rules CachedSelectorREEntry, exists bool) {
...
func (p *DNSProxy) UpdateAllowed(endpointID uint64, destPort, destProto uint16, newRules policy.L7DataMap) error {
	p.Lock()
	defer p.Unlock()
...
func (p *DNSProxy) UpdateAllowedFromSelectorRegexes(endpointID uint64, destPort, destProto uint16, newRules CachedSelectorREEntry) error {
	p.Lock()
	defer p.Unlock()
...
func (p *DNSProxy) CheckAllowed(endpointID uint64, destPort, destProto uint16, destID identity.NumericIdentity, destIP net.IP, name string) (allowed bool, err error) {
